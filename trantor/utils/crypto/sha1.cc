...
/**
 * Transforms the given state array using the SHA1 algorithm.
 *
 * @param state An array of 5 unsigned integers representing the state of the SHA1 hash.
 * @param buffer An array of 64 bytes representing the input data to be hashed.
 *
 * @throws None
 */
 ...
 *
 * @param ctx Pointer to the SHA1_CTX structure.
 *
 * @return void
 *
 * @throws None
...
/**
 * Updates the SHA1 context with the given data.
 *
 * @param ctx the SHA1 context to update
...
void trantor_sha1_update(SHA1_CTX* ctx, const unsigned char* data, size_t len) {
  ...
  if ((j + len) > 63) {
    ...
    trantor_sha1_transform(ctx->state, ctx->buffer);
    for (; i + 63 < len; i += 64) {
      trantor_sha1_transform(ctx->state, &data[i]);
    }
    j = 0;
  } else
    i = 0;
...
/**
 * Finalizes the SHA1 hash computation and stores the result in the provided
 * digest array.
 *
 * @param digest An array of 20 bytes to store the SHA1 digest.
 * @param ctx The SHA1 context containing the intermediate hash state.
 *
 * @return void
 *
 * @throws None
...
void trantor_sha1_final(unsigned char digest[20], SHA1_CTX* ctx) {
  ...
  for (i = 0; i < 8; i++) {
    finalcount[i] =
      (unsigned char)((ctx->count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255); /* Endian independent */
  }
#endif
  c = 0200;
  trantor_sha1_update(ctx, &c, 1);
  while ((ctx->count[0] & 504) != 448) {
    c = 0000;
    trantor_sha1_update(ctx, &c, 1);
  }
  trantor_sha1_update(ctx, finalcount, 8); /* Should cause a TrantorSHA1Transform() */
  for (i = 0; i < 20; i++) {
