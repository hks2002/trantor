...
/**
 * Converts an integer value to its string representation and stores it in the given buffer.
 *
 * @note Efficient Integer to String Conversions, by Matthew Wilson.
 *
 * @tparam T the type of the integer value
...
// Explicit instantiations for convertInt common types to avoid compiler warnings
template size_t convertInt(char buf[], short);
template size_t convertInt(char buf[], unsigned short);
template size_t convertInt(char buf[], int);
template size_t convertInt(char buf[], unsigned int);
template size_t convertInt(char buf[], long);
template size_t convertInt(char buf[], unsigned long);
template size_t convertInt(char buf[], long long);
template size_t convertInt(char buf[], unsigned long long);

/**
 * Converts an unsigned integer value to its hexadecimal representation and stores it in the given buffer.
 *
 * @param buf the buffer to store the hexadecimal representation
 ...
 * @throws None
 */
std::vector<std::string> splitString(std::string_view sv, std::string_view delimiter, bool acceptEmptyString) {
...
bool secureRandomBytes(void *data, size_t len) {
  ...
  // good enough.
  state.time = std::chrono::time_point_cast<std::chrono::milliseconds>(now).time_since_epoch().count();
