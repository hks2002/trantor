...
int main() {
  ...
  std::string  utf8PathAlt("\\tmp\\\xE4\xB8\xAD\xE6\x96\x87");  // Invalid, won't be changed
  ...
  if (nativePath1 != nativePath2) LOG_ERROR << "Error converting " << utf8PathStandard << " to native path";
  if (utf8PathStandard != trantor::utils::fromNativePath(nativePath1))
    LOG_ERROR << "Error converting " << utf8PathStandard << " from native to utf-8 path";
}
