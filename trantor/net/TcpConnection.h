...
#ifndef TRANTOR_TCP_CONNECTION_H
#define TRANTOR_TCP_CONNECTION_H
...
public:
   ...
   * data stream
   */
  virtual void sendStream(std::function<std::size_t(char *, std::size_t)> callback) = 0;  // (buffer, buffer size) ->
                                                                                          // size of data put in buffer
   ...
   * @note This method is only available for non-SSL connections.
   */
  [[deprecated("Use startEncryption(TLSPolicyPtr) instead")]] void startClientEncryption(
    ...
    policy->setUseOldTLS(useOldTLS).setValidate(validateCert).setHostname(hostname).setConfCmds(sslConfCmds);
...
#endif  // TRANTOR_TCP_CONNECTION_H
