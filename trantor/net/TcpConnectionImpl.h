...
#ifndef TRANTOR_TCP_CONNECTION_IMPL_H
#define TRANTOR_TCP_CONNECTION_IMPL_H

#include "TcpConnection.h"
#include "trantor/net/buffernode/BufferNode.h"
...
public:
  ...
  void               sendFile(const char *fileName, long long offset, long long length) override;
  void               sendFile(const wchar_t *fileName, long long offset, long long length) override;
  void               sendStream(std::function<std::size_t(char *, std::size_t)> callback) override;
  ...
  // }
  // set callbacks
  void setHighWaterMarkCallback(const HighWaterMarkCallback &cb, size_t markLen) override {
  ...
  MsgBuffer *getRecvBuffer() override {
    if (tlsProviderPtr_) return &tlsProviderPtr_->getRecvBuffer();
    return &readBuffer_;
  }

  std::string applicationProtocol() const override {
    if (tlsProviderPtr_) return tlsProviderPtr_->applicationProtocol();
    return "";
  }

  CertificatePtr peerCertificate() const override {
    if (tlsProviderPtr_) return tlsProviderPtr_->peerCertificate();
    return nullptr;
  }

  std::string sniName() const override {
    if (tlsProviderPtr_) return tlsProviderPtr_->sniName();
    return "";
  }
...
protected:
  ...
  void sendAsyncDataInLoop(const BufferNodePtr &node, const char *data, size_t len);
...
#endif  // TRANTOR_TCP_CONNECTION_IMPL_H
