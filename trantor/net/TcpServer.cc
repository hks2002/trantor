...
TcpServer::TcpServer(EventLoop *loop, const InetAddress &address, std::string name, bool reUseAddr, bool reUsePort)
  : loop_(loop),
    ...
    ioLoops_({loop}),
    numIoLoops_(1) {
  acceptorPtr_->setNewConnectionCallback([this](int fd, const InetAddress &peer) {
...
void TcpServer::newConnection(int sockfd, const InetAddress &peer) {
  ...
  newPtr->setConnectionCallback([this](const TcpConnectionPtr &connectionPtr) {
    if (connectionCallback_) connectionCallback_(connectionPtr);
  });
  newPtr->setWriteCompleteCallback([this](const TcpConnectionPtr &connectionPtr) {
    if (writeCompleteCallback_) writeCompleteCallback_(connectionPtr);
  });
    ...
    if (idleTimeout_ > 0) {
      for (EventLoop *loop : ioLoops_) {
        timingWheelMap_[loop] =
          std::make_shared<TimingWheel>(loop, idleTimeout_, 1.0F, idleTimeout_ < 500 ? idleTimeout_ + 1 : 100);
      }
    }
