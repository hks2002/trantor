...
static void defaultConnectionCallback(const TcpConnectionPtr &conn) {
  LOG_TRACE << conn->localAddr().toIpPort() << " -> " << conn->peerAddr().toIpPort() << " is "
...
void TcpClient::newConnection(int sockfd) {
  ...
  if (tlsPolicyPtr_) {
    ...
    conn = std::make_shared<TcpConnectionImpl>(loop_, sockfd, localAddr, peerAddr, tlsPolicyPtr_, sslContextPtr_);
  } else {
  ...
  conn->setSSLErrorCallback([weakSelf = std::move(weakSelf)](SSLError err) {
    ...
    if (self && self->sslErrorCallback_) self->sslErrorCallback_(err);
  });
