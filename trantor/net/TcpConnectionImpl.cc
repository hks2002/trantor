...
TcpConnectionImpl::TcpConnectionImpl(EventLoop         *loop,
  ...
  if (policy != nullptr) {
    tlsProviderPtr_ = newTLSProvider(this, std::move(policy), std::move(ctx));
...
TcpConnectionImpl::~TcpConnectionImpl() {
  ...
  if (!writeBufferList_.empty()) {
    LOG_DEBUG << "write node list size: " << writeBufferList_.size() << " first node is file? "
              << writeBufferList_.front()->isFile() << " first node is stream? " << writeBufferList_.front()->isStream()
              << " first node is async? " << writeBufferList_.front()->isAsync()
              << " first node size: " << writeBufferList_.front()->remainingBytes()
              << " buffered TLS data size: " << readableTlsBytes;
  } else if (readableTlsBytes != 0) {
    LOG_DEBUG << "write node list size: 0 buffered TLS data size: " << readableTlsBytes;
  }
  // send a close alert to peer if we are still connected
  if (tlsProviderPtr_ && status_ == ConnStatus::Connected) tlsProviderPtr_->close();
}
...
#ifdef _WIN32
      LOG_TRACE << "WSAENOTCONN or WSAECONNRESET, errno=" << errno << " fd=" << socketPtr_->fd();
#else
...
void TcpConnectionImpl::extendLife() {
  if (idleTimeout_ > 0) {
    ...
    lastTimingWheelUpdateTime_ = std::move(now);
...
void TcpConnectionImpl::writeCallback() {
  ...
  if (ioChannelPtr_->isWriting()) {
    ...
    if (tlsProviderPtr_ == nullptr || tlsProviderPtr_->getBufferedData().readableBytes() == 0) {
      ioChannelPtr_->disableWriting();
      if (closeOnEmpty_) {
        shutdown();
      }
  ...
  loop_->runInLoop([thisPtr]() {
    if (thisPtr->status_ == ConnStatus::Connected) {
      if (thisPtr->tlsProviderPtr_) {
        ...
        // connection just yet
        if (thisPtr->tlsProviderPtr_->getBufferedData().readableBytes() != 0 || !thisPtr->writeBufferList_.empty()) {
          thisPtr->closeOnEmpty_ = true;
          return;
        }
        thisPtr->tlsProviderPtr_->close();
      }
      if (thisPtr->tlsProviderPtr_ == nullptr && !thisPtr->writeBufferList_.empty()) {
        thisPtr->closeOnEmpty_ = true;
        return;
      }
...
void TcpConnectionImpl::forceClose() {
  ...
  loop_->runInLoop([thisPtr]() {
    if (thisPtr->status_ == ConnStatus::Connected || thisPtr->status_ == ConnStatus::Disconnecting) {
      ...
      if (thisPtr->tlsProviderPtr_) thisPtr->tlsProviderPtr_->close();
    }
  });
}
#ifndef _WIN32
...
#endif
{
  ...
  if (length > 0 && status_ == ConnStatus::Connected) {
    if (writeBufferList_.empty() || writeBufferList_.back()->isFile() || writeBufferList_.back()->isStream()) {
      writeBufferList_.push_back(BufferNode::newMemBufferNode());
    }
    writeBufferList_.back()->append(static_cast<const char *>(buffer) + sendLen, length);
    if (highWaterMarkCallback_ &&
        writeBufferList_.back()->remainingBytes() > static_cast<long long>(highWaterMarkLen_)) {
      highWaterMarkCallback_(shared_from_this(), writeBufferList_.back()->remainingBytes());
    }
...
void TcpConnectionImpl::send(std::string &&msg) {
  if (loop_->isInLoopThread()) {
    ...
    loop_->queueInLoop([thisPtr = shared_from_this(), msg = std::move(msg)]() {
      thisPtr->sendInLoop(msg.data(), msg.length());
    });
  }
}
...
void TcpConnectionImpl::sendFile(const char *fileName, long long offset, long long length) {
  assert(fileName);
#ifdef _WIN32
...
void TcpConnectionImpl::sendFile(const wchar_t *fileName, long long offset, long long length) {
  assert(fileName);
#ifndef _WIN32
  sendFile(utils::toNativePath(fileName).c_str(), offset, length);
#else
...
void TcpConnectionImpl::sendFile(BufferNodePtr &&fileNode) {
  ...
  if (loop_->isInLoopThread()) {
    if (writeBufferList_.empty()) {
      ...
      if (fileNode->remainingBytes() > 0 && n >= 0) writeBufferList_.push_back(std::move(fileNode));
      return;
    } else {
      writeBufferList_.push_back(std::move(fileNode));
    }
  } else {
    loop_->queueInLoop([thisPtr = shared_from_this(), node = std::move(fileNode)]() mutable {
      if (thisPtr->writeBufferList_.empty()) {
        auto n = thisPtr->sendNodeInLoop(node);
        if (node->remainingBytes() > 0 && n >= 0) thisPtr->writeBufferList_.push_back(std::move(node));
      } else {
...
void TcpConnectionImpl::sendStream(std::function<std::size_t(char *, std::size_t)> callback) {
  ...
  if (loop_->isInLoopThread()) {
    if (writeBufferList_.empty()) {
      ...
      if (node->remainingBytes() > 0 && n >= 0) writeBufferList_.push_back(std::move(node));
      return;
    }
  } else {
    loop_->queueInLoop([thisPtr = shared_from_this(), node = std::move(node)]() mutable {
...
ssize_t TcpConnectionImpl::sendNodeInLoop(const BufferNodePtr &nodePtr) {
  ...
  if (nodePtr->isFile() && !tlsProviderPtr_) {
    ...
    auto bytesSent = sendfile(socketPtr_->fd(),
                              nodePtr->getFd(),
                              nullptr,
                              static_cast<size_t>(toSend < kMaxSendBytes ? toSend : kMaxSendBytes));
    if (bytesSent > 0) {
...
#else
  int nWritten = ::send(socketPtr_->fd(), buffer, static_cast<int>(length), 0);
...
void TcpConnectionImpl::startEncryption(TLSPolicyPtr                                  policy,
                                        bool                                          isServer,
  ...
  tlsProviderPtr_    = newTLSProvider(this, std::move(policy), std::move(sslContextPtr));
  tlsProviderPtr_->setWriteCallback(onSslWrite);
  tlsProviderPtr_->setErrorCallback(onSslError);
  tlsProviderPtr_->setHandshakeCallback(onHandshakeFinished);
  tlsProviderPtr_->setMessageCallback(onSslMessage);
...
void TcpConnectionImpl::onSslError(TcpConnection *self, SSLError err) {
  if (self->sslErrorCallback_) self->sslErrorCallback_(err);
  self->forceClose();
}
...
void TcpConnectionImpl::onSslMessage(TcpConnection *self, MsgBuffer *buffer) {
  if (self->recvMsgCallback_) self->recvMsgCallback_(((TcpConnectionImpl *)self)->shared_from_this(), buffer);
}
...
public:
  explicit AsyncStreamImpl(std::function<bool(const char *, size_t)> callback) : callback_(std::move(callback)) {}
  ...
  ~AsyncStreamImpl() override {
    if (callback_) callback_(nullptr, 0);
  }
...
AsyncStreamPtr TcpConnectionImpl::sendAsyncStream(bool disableKickoff) {
  ...
  auto                             asyncStream     = std::make_unique<AsyncStreamImpl>(
    [asyncStreamNode, weakPtr = std::move(weakPtr)](const char *data, size_t len) -> bool {
      ...
      if (thisPtr->loop_->isInLoopThread()) {
        ...
        if (data) {
          ...
          thisPtr->loop_->queueInLoop([thisPtr, asyncStreamNode, buffer = std::move(buffer)]() {
            thisPtr->sendAsyncDataInLoop(asyncStreamNode, buffer.data(), buffer.length());
          });
        } else {
          thisPtr->loop_->queueInLoop([thisPtr, asyncStreamNode]() {
            thisPtr->sendAsyncDataInLoop(asyncStreamNode, nullptr, 0);
          });
        }
      }
      return true;
  ...
  if (loop_->isInLoopThread()) {
    ...
    loop_->queueInLoop(
      [this, thisPtr = shared_from_this(), node = std::move(asyncStreamNode), disableKickoff]() mutable {
        if (disableKickoff) {
          auto entry = kickoffEntry_.lock();
          if (entry) {
            entry->reset();
...
void TcpConnectionImpl::sendAsyncDataInLoop(const BufferNodePtr &node, const char *data, size_t len) {
  loop_->assertInLoopThread();
  if (data) {
    if (len > 0) {
      if (!writeBufferList_.empty() && node == writeBufferList_.front() && node->remainingBytes() == 0) {
        auto nWritten = writeInLoop(data, len);
        if (nWritten < 0) {
          LOG_TRACE << "write error";
          return;
    ...
    // stream is closed
    node->done();
    if (!writeBufferList_.empty() && node == writeBufferList_.front() && !ioChannelPtr_->isWriting())
