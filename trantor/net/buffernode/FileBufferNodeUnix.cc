...
public:
  FileBufferNode(const char *fileName, long long offset, long long length) {
    ...
    if (length == 0) {
      if (offset >= filestat.st_size) {
        LOG_ERROR << "The file size is " << filestat.st_size << " bytes, but the offset is " << offset
  ...
  /**
   * @brief Indicate whether the buffer is a file.
   * @return
   */
  ...
  void getData(const char *&data, size_t &len) override {
    if (msgBufferPtr_ == nullptr) {
      msgBufferPtr_ =
        std::make_unique<MsgBuffer>((std::min)(kMaxSendFileBufferSize, static_cast<size_t>(fileBytesToSend_)));
    }
    if (msgBufferPtr_->readableBytes() == 0 && fileBytesToSend_ > 0 && sendFd_ >= 0) {
      msgBufferPtr_->ensureWritableBytes((std::min)(kMaxSendFileBufferSize, static_cast<size_t>(fileBytesToSend_)));
      auto n = read(sendFd_, msgBufferPtr_->beginWrite(), msgBufferPtr_->writableBytes());
      if (n > 0) {
  ...
  /**
   * @brief Remain bytes in the buffer.
   * @return
   */
...
BufferNodePtr BufferNode::newFileBufferNode(const char *fileName, long long offset, long long length) {
