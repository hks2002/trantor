...
public:
  FileBufferNode(const wchar_t *fileName, long long offset, long long length) {
#if UWP
    sendHandle_ = CreateFile2(fileName, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr);
    ...
    if (length == 0) {
      if (offset >= fileSize.QuadPart) {
        LOG_ERROR << "The file size is " << fileSize.QuadPart << " bytes, but the offset is " << offset
    ...
    msgBufferPtr_ = std::make_unique<MsgBuffer>(kMaxSendFileBufferSize < fileBytesToSend_ ? kMaxSendFileBufferSize
                                                                                          : fileBytesToSend_);
  }
  ...
  /**
   * @brief Indicate this buffer is a file.
   * @return
   */
  ...
  /**
   * @brief Indicate the buffer is available.
   * @return
   */
  ...
  /**
   * @brief Get the file handle. Windows does not support this function.
   * @return
   */
  ...
  void getData(const char *&data, size_t &len) override {
    if (msgBufferPtr_->readableBytes() == 0 && fileBytesToSend_ > 0 && sendHandle_ != INVALID_HANDLE_VALUE) {
      msgBufferPtr_->ensureWritableBytes(kMaxSendFileBufferSize < fileBytesToSend_ ? kMaxSendFileBufferSize
                                                                                   : fileBytesToSend_);
      DWORD n = 0;
      if (!ReadFile(sendHandle_, msgBufferPtr_->beginWrite(), (uint32_t)msgBufferPtr_->writableBytes(), &n, nullptr)) {
  ...
  void retrieve(size_t len) override {
    ...
    if (fileBytesToSend_ < 0) fileBytesToSend_ = 0;
  }
...
BufferNodePtr BufferNode::newFileBufferNode(const wchar_t *fileName, long long offset, long long length) {
