...
#include "trantor/net/tlsprovider/TLSProvider.h"
...
inline bool verifyCommonName(X509 *cert, const std::string &hostname) {
  ...
  if (subjectName != nullptr) {
    ...
    return utils::verifySslName(std::string(name.begin(), name.begin() + length), hostname);
  }
...
inline bool verifyAltName(X509 *cert, const std::string &hostname) {
  bool good = false;
  auto altNames =
    static_cast<const struct stack_st_GENERAL_NAME *>(X509_get_ext_d2i(cert, NID_subject_alt_name, nullptr, nullptr));
  ...
  if (altNames) {
    ...
    for (int i = 0; i < numNames && !good; i++) {
      ...
      good          = utils::verifySslName(std::string(name, name + name_len), hostname);
    }
  }
...
static bool validatePeerCertificate(SSL               *ssl,
  ...
  if (isServer) {
    bool domainIsValid = verifyCommonName(cert, hostname) || verifyAltName(cert, hostname);
    if (!domainIsValid) return false;
  }

  auto result = SSL_get_verify_result(ssl);
  if (result == X509_V_ERR_CERT_NOT_YET_VALID || result == X509_V_ERR_CERT_HAS_EXPIRED) {
    // What happens if cert is self-signed and expired?
    LOG_TRACE << "cert error code: " << result << ", date validation failed";
    return false;
  }
...
static int serverSelectProtocol(SSL                  *ssl,
  ...
  if (protocols->empty()) return SSL_TLSEXT_ERR_NOACK;
  ...
  for (auto &protocol : *protocols) {
    ...
    while (cur < end) {
      ...
      if (protocol.size() == len && memcmp(cur, protocol.data(), len) == 0) {
        *out    = cur;
        *outlen = len;
...
namespace trantor {
struct SSLContext {
  SSLContext(bool useOldTLS, const std::vector<std::pair<std::string, std::string>> &sslConfCmds, bool server)
    : isServer(server) {
...
#define SSL_METHOD TLS_method
    ...
    if (ctx_ == nullptr) throw std::runtime_error("Failed to create SSL context");
    if (sslConfCmds.size() != 0) LOG_WARN << "LibreSSL does not support SSL configuration commands";

    if (!useOldTLS) SSL_CTX_set_min_proto_version(ctx_, TLS1_2_VERSION);
#else
    ctx_ = SSL_CTX_new(SSL_METHOD());
    if (ctx_ == nullptr) throw std::runtime_error("Failed to create SSL context");
    ...
    for (const auto &cmd : sslConfCmds) SSL_CONF_cmd(cctx, cmd.first.data(), cmd.second.data());
...
#else
      const auto opt = SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3;
...
public:
  ...
  void store(const std::string &hostname, InetAddress peerAddr, SSL_SESSION *session, EventLoop *loop) {
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
    {
  ...
  void removeExcessSession() {
    ...
    if (sessions_.size() < size_t(maxSessions_ + mexExtendSize_)) return;
  ...
  std::unordered_map<std::string, std::list<SessionData>::iterator> sessionMap_;
};
...
struct OpenSSLProvider : public TLSProvider, public NonCopyable {
  OpenSSLProvider(TcpConnection *conn, TLSPolicyPtr policy, SSLContextPtr ctx)
    ...
    if (!policyPtr_->getHostname().empty()) SSL_set_tlsext_host_name(ssl_, policyPtr_->getHostname().c_str());
  }
  ...
  virtual void startEncryption() override {
    if (contextPtr_->isServer) {
      ...
      if (!protocols.empty()) {
        ...
        SSL_set_alpn_protos(ssl_, (const unsigned char *)(alpnList.data()), (unsigned int)alpnList.size());
      }
  ...
  virtual void recvData(MsgBuffer *buffer) override {
    LOG_TRACE << "Received " << buffer->readableBytes() << " bytes from lower layer";
    ...
    while (buffer->readableBytes() > 0) {
      ...
      if (!SSL_is_init_finished(ssl_)) {
        ...
        if (handshakeDone) processApplicationData();
      } else {
  ...
  virtual ssize_t sendData(const char *data, size_t len) override {
    ...
    while (hasSent < len && getBufferedData().readableBytes() == 0) {
      ...
      if (trunkLen > maxSend) trunkLen = maxSend;
  ...
  bool processHandshake() {
    ...
    if (ret == 1) {
      ...
      if (contextPtr_->isServer) {
        const char *sniName = SSL_get_servername(ssl_, TLSEXT_NAMETYPE_host_name);
        if (sniName) setSniName(sniName);

        const unsigned char *alpn    = nullptr;
        unsigned int         alpnlen = 0;
        SSL_get0_alpn_selected(ssl_, &alpn, &alpnlen);
        if (alpn) setApplicationProtocol(std::string((char *)alpn, alpnlen));
      } else {
        ...
        if (SSL_SESSION_is_resumable(session)) {
          ...
          if (reused == 0) sessionManager.store(sniName_, conn_->peerAddr(), session, loop_);
        }
#endif
      }
      ...
      if (cert) setPeerCertificate(std::make_shared<OpenSSLCertificate>(cert));

      if (needCert) {
        if (cert) {
          bool valid = internal::validatePeerCertificate(ssl_,
                                                         cert,
      ...
      if (handshakeCallback_) handshakeCallback_(conn_);
      ...
      if (err == SSL_ERROR_WANT_READ) {
        ...
        if (!processedHandshakeError_)
          ...
          return false;
        LOG_TRACE << "SSL handshake error: " << ERR_error_string(ERR_get_error(), NULL);
        conn_->shutdown();
        handleSSLError(SSLError::kSSLHandshakeError);
      }
    }
  ...
  void processApplicationData() {
    ...
    while (true) {
      ...
      recvBuffer_.ensureWritableBytes((std::min)(maxSingleRead, (size_t)pending));
      // clamp to int, because that's what SSL_read accepts
      const size_t wrtibleSize   = (std::min)(maxWritibleBytes, recvBuffer_.writableBytes());
      ...
      if (n == 0 && (shutdownState & SSL_RECEIVED_SHUTDOWN)) {
        ...
        if (messageCallback_) messageCallback_(conn_, &recvBuffer_);
      } else if (n <= 0) {
  ...
  void handleSSLError(SSLError error) {
    ...
    if (!processedSslError_)
      ...
      return;
    if (errorCallback_) errorCallback_(conn_, error);
  }
...
std::shared_ptr<TLSProvider> trantor::newTLSProvider(TcpConnection *conn, TLSPolicyPtr policy, SSLContextPtr ctx) {
  return std::make_shared<OpenSSLProvider>(conn, std::move(policy), std::move(ctx));
}

SSLContextPtr trantor::newSSLContext(const TLSPolicy &policy, bool isServer) {
  auto ctx = std::make_shared<SSLContext>(policy.getUseOldTLS(), policy.getConfCmds(), isServer);
  if (!policy.getCertPath().empty() && !policy.getKeyPath().empty()) {
    if (SSL_CTX_use_certificate_chain_file(ctx->ctx(), policy.getCertPath().data()) <= 0) {
      throw std::runtime_error("Failed to load certificate " + policy.getCertPath());
    }
    if (SSL_CTX_use_PrivateKey_file(ctx->ctx(), policy.getKeyPath().data(), SSL_FILETYPE_PEM) <= 0) {
  ...
  if (!policy.getCaPath().empty()) {
    if (isServer) {
      if (SSL_CTX_load_verify_locations(ctx->ctx(), policy.getCaPath().data(), nullptr) <= 0) {
        throw std::runtime_error("Failed to load CA certificate");
      }

      STACK_OF(X509_NAME) *cert_names = SSL_load_client_CA_file(policy.getCaPath().data());
      if (cert_names == nullptr) {
        throw std::runtime_error("Not CA names found in file");
      }
      SSL_CTX_set_client_CA_list(ctx->ctx(), cert_names);
      SSL_CTX_set_verify(ctx->ctx(), SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, nullptr);
      LOG_TRACE << "Finished loading custom CA";
    } else {
      auto *store = X509_STORE_new();
      if (!X509_STORE_load_locations(store, policy.getCaPath().data(), nullptr)) {
        throw std::runtime_error("Failed to load CA certificate");
      }
  ...
  if (!policy.getAlpnProtocols().empty() && isServer) {
    SSL_CTX_set_alpn_select_cb(ctx->ctx(), internal::serverSelectProtocol, (void *)&policy.getAlpnProtocols());
  }
  ...
  int status = SSL_CTX_set_cipher_list(ctx->ctx(), "MEDIUM:HIGH:!aNULL!MD5:!RC4!3DES");
  if (status != 1) throw std::runtime_error("Failed to select secure ciphers");
