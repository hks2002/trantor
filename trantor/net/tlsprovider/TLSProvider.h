...
struct TLSProvider {
  TLSProvider(TcpConnection* conn, TLSPolicyPtr policy, SSLContextPtr ctx)
    : conn_(conn), policyPtr_(std::move(policy)), contextPtr_(std::move(ctx)), loop_(conn_->getLoop()) {}
  ...
  bool sendBufferedData() {
    ...
    auto n = writeCallback_(conn_, writeBuffer_.peek(), writeBuffer_.readableBytes());
    if (n == -1) {
      LOG_ERROR << "WTF! Failed to send buffered data. Error: " << strerror(errno);
      return false;
...
std::shared_ptr<TLSProvider> newTLSProvider(TcpConnection* conn, TLSPolicyPtr policy, SSLContextPtr ctx);
}  // namespace trantor
