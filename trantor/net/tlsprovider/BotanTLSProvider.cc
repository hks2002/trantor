#include "trantor/net/tlsprovider/BotanTLSProvider.h"
...
public:
  ...
  std::vector<Botan::X509_Certificate> find_cert_chain(
    ...
    auto key_algo = cert_->subject_public_key_algo().oid().to_formatted_string();
    auto it       = std::find(cert_key_types.begin(), cert_key_types.end(), key_algo);
    if (it == cert_key_types.end()) return {};
    return {*cert_};
  }
...
public:
  BotanTLSProvider(TcpConnection *conn, TLSPolicyPtr policy, SSLContextPtr ctx)
  ...
  virtual void recvData(MsgBuffer *buffer) override {
    ...
    try {
      ...
      channel_->received_data((const uint8_t *)buffer->peek(), buffer->readableBytes());
  ...
  virtual void startEncryption() override {
    ...
    if (certStorePtr == nullptr) {
      std::call_once(systemCertStoreInitFlag, []() {
        systemCertStore = std::make_unique<Botan::System_Certificate_Store>();
      });
      certStorePtr = systemCertStore.get();
    }
    credsPtr_ = std::make_shared<Credentials>(contextPtr_->key, contextPtr_->cert.get(), certStorePtr);
    if (policyPtr_->getConfCmds().empty() == false) LOG_WARN << "BotanTLSConnectionImpl does not support sslConfCmds.";
    ...
    std::call_once(sessionManagerInitFlag, []() {
      ...
      sessionManager    = std::make_shared<Botan::TLS::Session_Manager_In_Memory>(sessionManagerRng);
    });
    if (rng == nullptr) rng = std::make_shared<Botan::AutoSeeded_RNG>();
    ...
    if (contextPtr_->isServer) {
      ...
      validationPolicy_->requireClientCert_ = contextPtr_->requireClientCert;
      channel_ =
        std::make_unique<Botan::TLS::Server>(std::move(fakeThis), sessionManager, credsPtr_, validationPolicy_, rng);
    } else {
      validationPolicy_->requireClientCert_ = contextPtr_->requireClientCert;
      // technically Botan2 does support TLS 1.0 and 1.1, but Botan3 does
      // not. So we just disable them to keep compatibility.
      if (policyPtr_->getUseOldTLS()) LOG_WARN << "Old TLS not supported by Botan (only >= TLS 1.2)";
      channel_ = std::make_unique<Botan::TLS::Client>(
        ...
        validationPolicy_,
        rng,
        Botan::TLS::Server_Information(policyPtr_->getHostname(), conn_->peerAddr().toPort()),
  ...
  void tls_emit_data(std::span<const uint8_t> data) override {
    ...
    if (n == ssize_t(data.size_bytes())) return;
    if (n == -1) n = 0;
    appendToWriteBuffer((const char *)data.data() + n, data.size_bytes() - n);
  }
  ...
  std::string tls_server_choose_app_protocol(const std::vector<std::string> &client_protos) override {
    ...
    if (policyPtr_->getAlpnProtocols().empty() || client_protos.empty()) return "";

    for (auto const &proto : client_protos) {
      if (std::find(policyPtr_->getAlpnProtocols().begin(), policyPtr_->getAlpnProtocols().end(), proto) !=
    ...
    throw Botan::TLS::TLS_Exception(
      ...
      "No supported application protocol found. Client offered: " + join(client_protos, ", ") +
        " but we support: " + join(policyPtr_->getAlpnProtocols(), ", "));
  }

  void tls_alert(Botan::TLS::Alert alert) override {
    if (alert.type() == Botan::TLS::Alert::CloseNotify) {
      LOG_TRACE << "TLS close notify received";
      if (closeCallback_) closeCallback_(conn_);
    } else {
      if (errorCallback_) errorCallback_(conn_, SSLError::kSSLProtocolError);
    }
  }
  ...
  void tls_verify_cert_chain(const std::vector<Botan::X509_Certificate>              &certs,
    ...
    if (policyPtr_->getValidate() && !policyPtr_->getAllowBrokenChain())
      ...
      if (certs.size() == 0)
        throw Botan::TLS::TLS_Exception(Botan::TLS::Alert::NoCertificate,
      ...
      Botan::Path_Validation_Restrictions  restrictions(false,  // require revocation
                                                       validationPolicy_->minimum_signature_strength());

      auto                                 now = std::chrono::system_clock::now();
      const auto status = Botan::PKIX::check_chain(selfSigned, now, hostname, usage, restrictions);
      ...
      if (result != Botan::Certificate_Status_Code::OK)
        throw Botan::TLS::TLS_Exception(Botan::TLS::Alert::BadCertificate,
                                        std::string("Certificate validation failed: ") + Botan::to_string(result));
    }

    if (certs.size() > 0) setPeerCertificate(std::make_shared<BotanCertificate>(certs[0]));
  }
...
std::shared_ptr<TLSProvider> trantor::newTLSProvider(TcpConnection *conn, TLSPolicyPtr policy, SSLContextPtr ctx) {
  return std::make_shared<BotanTLSProvider>(conn, std::move(policy), std::move(ctx));
}
...
SSLContextPtr trantor::newSSLContext(const TLSPolicy &policy, bool server) {
  ...
  if (!policy.getCertPath().empty()) {
    ctx->cert = std::make_unique<Botan::X509_Certificate>(policy.getCertPath());
  }

  if (policy.getValidate() && policy.getAllowBrokenChain()) {
    if (!policy.getCaPath().empty()) {
      ctx->certStore = std::make_shared<Botan::Flatfile_Certificate_Store>(policy.getCaPath());
      if (server) ctx->requireClientCert = true;
    } else if (policy.getUseSystemCertStore()) {
      static auto systemCertStore = std::make_shared<Botan::System_Certificate_Store>();
