...
TimerId TimerQueue::addTimer(const TimerCallback &cb, const TimePoint &when, const TimeInterval &interval) {
  TimerPtr timerPtr = std::make_shared<Timer>(cb, when, interval);
...
TimerId TimerQueue::addTimer(TimerCallback &&cb, const TimePoint &when, const TimeInterval &interval) {
  TimerPtr timerPtr = std::make_shared<Timer>(std::move(cb), when, interval);
...
void TimerQueue::reset(const TimerPtrVector &expired, const TimePoint &now) {
  loop_->assertInLoopThread();
  for (auto const &timerPtr : expired) {
...
TimerPtrVector TimerQueue::getExpired(const TimePoint &now) {
  TimerPtrVector expired;
  while (!timers_.empty()) {
