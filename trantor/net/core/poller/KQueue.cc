...
KQueue::KQueue(EventLoop *loop) : Poller(loop), kqfd_(kqueue()), events_(kInitEventListSize) {
  assert(kqfd_ >= 0);
}
...
void KQueue::poll(int timeoutMs, ChannelList *activeChannels) {
  ...
  int numEvents   = kevent(kqfd_, NULL, 0, events_.data(), static_cast<int>(events_.size()), &timeout);
