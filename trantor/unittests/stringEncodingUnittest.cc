...
TEST(pathConversion, fromUtf8a) {
  ...
  EXPECT_EQ(out, widePath) << "Error converting " << utf8Path << " to wide string";
}

TEST(pathConversion, fromUtf8b) {
  auto out = utils::fromUtf8(utf8AltPath);
  EXPECT_EQ(out, windowsPath) << "Error converting " << utf8AltPath << " to wide string";
}

TEST(pathConversion, toUtf8a) {
  auto out = utils::toUtf8(widePath);
  EXPECT_EQ(out, utf8Path) << "Error converting " << widePath << " from wide string to utf-8";
}

TEST(pathConversion, toUtf8b) {
  auto out = utils::toUtf8(windowsPath);
  EXPECT_EQ(out, utf8AltPath) << "Error converting " << windowsPath << " from wide string to utf-8";
}

TEST(pathConversion, fromWidePath1) {
  auto out = utils::fromWidePath(widePath);
  EXPECT_EQ(out, utf8Path) << "Error converting " << widePath << " from wide path to utf-8";
}
...
TEST(pathConversion, toWidePath2) {
  ...
  EXPECT_EQ(out, windowsPath) << "Error converting " << utf8AltPath << " from utf-8 path to wide string";
}

TEST(pathConversion, toNativePath1) {
  auto out = utils::toNativePath(utf8Path);
  EXPECT_EQ(out, nativePath) << "Error converting " << utf8Path << " from utf-8 path to native path";
}
...
TEST(pathConversion, toNativePath2) {
  ...
  EXPECT_EQ(out, windowsPath) << "Error converting " << utf8AltPath << " from utf-8 path to wide string";
#else   // _WIN32
  EXPECT_EQ(out, utf8AltPath) << "Error converting " << utf8AltPath << " from utf-8 path (should be a noop)";
#endif  // _WIN32
}
...
#ifdef _WIN32
  EXPECT_EQ(out, windowsPath) << "Error converting " << windowsPath << " from wide path (should be a noop)";
#else   // _WIN32
  EXPECT_EQ(out, utf8AltPath) << "Error converting " << windowsPath << " from wide path to utf-8";
#endif  // _WIN32
}
...
#ifdef _WIN32
  EXPECT_EQ(out, widePath) << "Error converting " << widePath << " from wide path (should be a noop)";
#else   // _WIN32
  EXPECT_EQ(out, utf8Path) << "Error converting " << widePath << " from wide path to utf-8";
#endif  // _WIN32
}
...
TEST(pathConversion, fromNativePath1) {
  ...
  EXPECT_EQ(out, utf8Path) << "Error converting " << nativePath << " from native path to utf-8";
}

TEST(pathConversion, fromNativePath2) {
  auto out = utils::fromNativePath(utf8Path);
  EXPECT_EQ(out, utf8Path) << "Error converting " << utf8Path << " from utf-8 path (should be a noop)";
}
